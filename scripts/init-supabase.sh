...
create_project() {
    ...
    local create_cmd="supabase projects create \"$project_name\" --db-password \"$db_password\" --region $region"
    ...
    if [ "$plan" = "pro" ]; then
        create_cmd="$create_cmd --plan pro"
    fi
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ -z "$project_ref" ]; then
            ...
            project_ref=$(echo "$output" | grep -oP '[a-z]{20}' | head -n1)
        fi
        ...
        if [ -n "$project_ref" ]; then
            ...
            print_warn "Could not extract project reference automatically"
        fi
    else
