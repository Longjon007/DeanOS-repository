...
1. **Stabilize the product**
   - Freeze the feature set for both the Next.js web app (`web/`) and Expo mobile app (`app/`), then run:
     ...
     - In `app/`: `expo start` for a smoke test (or `expo run:android` / `expo run:ios` when running on device builds), then open the app to log in and issue a sample request
...
3. **Package and deploy**
   - Web: In `web/`: run `npm install` (from a clean checkout), then `npm run lint && npm run build`, and deploy the generated build to the chosen host with a branded domain and TLS.
   - Mobile:
