...
1. **Stabilize the product**
   - Freeze the feature set for both the Next.js web app (`web/`) and Expo mobile app (`app/`), then run `./health_check.sh`, `npm run lint && npm run build` in `web/`, and a basic Expo build/smoke test for `app/`.
...
2. **Harden security and compliance**
   ...
   - Publish required legal pages (Terms of Service, Privacy) and replace the current template in SECURITY.md with Hyperion-specific policies before aligning data handling accordingly.
3. **Package and deploy**
   ...
   - Mobile: Produce signed store builds through Expo (Android/iOS), supply icons/screenshots/descriptions, and validate deep links plus API calls to Supabase/AI endpoints.
4. **Enable commerce**
